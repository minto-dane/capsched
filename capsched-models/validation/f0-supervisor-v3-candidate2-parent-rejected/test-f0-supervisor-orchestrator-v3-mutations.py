...
def main() -> None:
    ...
    assert tuple(
        (event.attempt, event.event_kind, event.attack)
        for event in second_rejected.store_security_events
    ) == (
        (1, "ATTEMPT", "GRANT_REPLAY"),
        (1, "REJECTED", "GRANT_REPLAY"),
        (2, "ATTEMPT", "PUBLICATION_ROLLBACK"),
        (2, "REJECTED", "PUBLICATION_ROLLBACK"),
    )
    ...
    assert abandonment_conflict.phase == "TERMINAL"
    assert abandonment_conflict.publication == "ABANDONMENT_STALE_REJECTED"
