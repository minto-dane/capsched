#!/usr/bin/env python3
"""Actor-separated parent orchestration LTS for supervisor v3 candidate-2."""
...
_allow_writes(
    ...
    "pending_attack", "store_attack_attempts", "store_security_events",
)
_allow_writes(
...
_allow_writes(
    ...
    "owner_failure_notice", "owner_state", "parent_receipts", "phase", "publication",
)
_allow_writes(
...
_allow_writes(
    ...
    "pending_attack", "store_attack_rejections", "store_security_events",
)
_allow_writes(
    ("ADV-029B-SUCCEED-STORE-BYPASS",),
    "assurance_breach", "pending_attack", "phase", "publication",
)
...
if set(ACTION_WRITE_FIELDS) != set(ACTION_SPECS):
    raise RuntimeError(
        "orchestrator action write policy does not exactly cover the action registry"
    )
...
def store_security_wf(state: OrchestratorState) -> bool:
    ...
    for sequence, event in enumerate(state.store_security_events, start=1):
        ...
        if event.event_kind == "ATTEMPT":
            if len(attempts) != len(rejections) or event.attempt != len(attempts) + 1:
                return False
    ...
    if (
        ...
        or len(attempts) > MAX_STORE_ATTACK_ATTEMPTS
    ):
        return False
...
def owner_failure_wf(state: OrchestratorState) -> bool:
    ...
    if owner_receipts:
        return (
            ...
            and owner_receipts[0].payload == state.parent_grant.parent_run_id
        )
    return (
        state.parent_ledger == "SEALED"
...
def publication_wf(state: OrchestratorState) -> bool:
    ...
    if state.publication == "ABANDONMENT_STALE_REJECTED":
        return (
            state.phase == "TERMINAL"
            and state.publication_ack is None
...
def store_next(state: OrchestratorState) -> tuple[OrchEdge, ...]:
    ...
    if state.pending_attack != "NONE":
        ...
        updated = replace(updated, pending_attack="NONE")
        edges.append(
    ...
    if (
        ...
        conflict = replace(
            state,
            phase="TERMINAL",
            publication="ABANDONMENT_STALE_REJECTED",
            store_head_generation=expected + 1,
        )
        edges.append(
            _edge(
...
def adversary_next(state: OrchestratorState) -> tuple[OrchEdge, ...]:
    ...
    if (
        ...
        and len(state.store_attack_attempts) < MAX_STORE_ATTACK_ATTEMPTS
    ):
        def attempt_edge(action_id: str, attack: str) -> OrchEdge:
            updated = _append_store_security_event(
                state,
                actor,
                "ATTEMPT",
            ...
            updated = replace(updated, pending_attack=attack)
            return _edge(action_id, actor, state, updated)

        if state.parent_grant is not None:
            edges.append(attempt_edge("ADV-027-REPLAY-GRANT", "GRANT_REPLAY"))
...
def explore() -> dict[str, object]:
    ...
    return {
        ...
        "store_attack_attempt_bound": MAX_STORE_ATTACK_ATTEMPTS,
        "store_security_history_exact": True,
        "repeated_store_attack_state_count": repeated_store_attacks,
