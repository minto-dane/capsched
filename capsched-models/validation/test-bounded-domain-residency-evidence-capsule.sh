...
CAPSULE=$(realpath "$1")
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/residency-evidence-test.XXXXXX")
...
printf 'tamper\n' >>"$TMP_DIR/direct/raw/tlc/safe-admission.log"
if python3 "$COLLECTOR" verify \
	--capsule "$TMP_DIR/direct" >/dev/null 2>&1; then
...
target = root / "raw/tlc/safe-admission.status"
mutated = b"12\n"
...
row = next(
    ...
    if item["capsule_path"] == "raw/tlc/safe-admission.status"
)
