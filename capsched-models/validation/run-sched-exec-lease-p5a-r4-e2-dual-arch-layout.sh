...
SOURCE_GATE="$WORKSPACE_DIR/build/source-check/sched-exec-lease-p5a-r4-e2-source-gate/20260717T-p5a-r4-e2-source-gate-r1/result.json"
RUN_ID=${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}
OUT_DIR="$WORKSPACE_DIR/build/source-check/sched-exec-lease-p5a-r4-e2-dual-arch-layout/$RUN_ID"
BUILD_ROOT=${DOMAINLEASE_P5AR4_E2_BUILD_ROOT:-"$WORKSPACE_DIR/build/DomainLeaseLinux.volume/builds/p5a-r4-e2-dual-arch/$RUN_ID"}
PROGRESS_FILE=${PROGRESS_FILE:-}

die() { printf 'error: %s\n' "$*" >&2; exit 1; }
progress() { [ -z "$PROGRESS_FILE" ] || printf '%s\n' "$*" > "$PROGRESS_FILE"; printf '[progress] %s\n' "$*"; }

for cmd in awk diff gcc git grep jq make nm nproc readelf sed sha256sum sort strings wc x86_64-linux-gnu-gcc x86_64-linux-gnu-nm x86_64-linux-gnu-readelf; do
...
jq -e '
  ...
  .candidate_parent == "5e1ca3037e34823d1ba0cdd1dc04161fac170280" and
  .exact_two_file_boundary == true and
  .direct_primary_child == true and
  ...
  .expected_private_symbol_count == 58 and
...
[ "$source_gate_sha" = '9e79d3e58151960b397a715116eb545de4c1ecc1988e619b88139022f6395a82' ] || die "source gate hash mismatch: $source_gate_sha"
...
prepare_config()
{
	...
	case "$mode" in
		baseline)
			...
			;;
		private-off)
			"$source/scripts/config" --file "$out/.config" -e SCHED_EXEC_LEASE_LAYOUT_PROBE -d SCHED_EXEC_LEASE_R4_LAYOUT_PROBE
			;;
		private-on)
			"$source/scripts/config" --file "$out/.config" -e SCHED_EXEC_LEASE_LAYOUT_PROBE -e SCHED_EXEC_LEASE_R4_LAYOUT_PROBE
			;;
		normal)
			"$source/scripts/config" --file "$out/.config" -d SCHED_EXEC_LEASE_LAYOUT_PROBE -d SCHED_EXEC_LEASE_R4_LAYOUT_PROBE
			;;
	...
	grep -q '^CONFIG_IRQ_WORK=y$' "$out/.config" || die "$log_prefix IRQ work config missing"
	case "$mode" in
		baseline|private-off)
			grep -q '^CONFIG_SCHED_EXEC_LEASE_LAYOUT_PROBE=y$' "$out/.config" || die "$log_prefix existing probe missing"
			! grep -q '^CONFIG_SCHED_EXEC_LEASE_R4_LAYOUT_PROBE=y$' "$out/.config" || die "$log_prefix R4 probe unexpectedly enabled"
			;;
		private-on)
			grep -q '^CONFIG_SCHED_EXEC_LEASE_LAYOUT_PROBE=y$' "$out/.config" || die "$log_prefix existing probe missing"
			grep -q '^CONFIG_SCHED_EXEC_LEASE_R4_LAYOUT_PROBE=y$' "$out/.config" || die "$log_prefix R4 probe missing"
			;;
		normal)
			! grep -q '^CONFIG_SCHED_EXEC_LEASE_LAYOUT_PROBE=y$' "$out/.config" || die "$log_prefix existing probe enabled"
			! grep -q '^CONFIG_SCHED_EXEC_LEASE_R4_LAYOUT_PROBE=y$' "$out/.config" || die "$log_prefix R4 probe enabled"
			;;
	esac
}
...
validate_arch()
{
	...
	extract_symbols "$nm_cmd" "$baseline_exec" sched_exec_r4l_ "$arch_out/baseline-private.tsv"
	extract_symbols "$nm_cmd" "$off_exec" sched_exec_r4l_ "$arch_out/private-off-private.tsv"
	extract_symbols "$nm_cmd" "$on_exec" sched_exec_r4l_ "$arch_out/private-on-private.tsv"
	extract_symbols "$nm_cmd" "$normal_exec" sched_exec_r4l_ "$arch_out/normal-private.tsv"
	[ "$(wc -l < "$arch_out/baseline-private.tsv" | tr -d ' ')" = 0 ] || die "$label primary baseline contains R4 symbols"
	[ "$(wc -l < "$arch_out/private-off-private.tsv" | tr -d ' ')" = 0 ] || die "$label private-off contains R4 symbols"
	[ "$(wc -l < "$arch_out/normal-private.tsv" | tr -d ' ')" = 0 ] || die "$label normal contains R4 symbols"
	private_count=$(wc -l < "$arch_out/private-on-private.tsv" | tr -d ' ')
	[ "$private_count" = 58 ] || die "$label private symbol count: $private_count"
	...
	strings "$off_exec" > "$arch_out/private-off-strings.txt"
	strings "$normal_exec" > "$arch_out/normal-strings.txt"
	for disabled_file in "$arch_out/private-off-relocations.txt" "$arch_out/normal-relocations.txt" "$arch_out/private-off-strings.txt" "$arch_out/normal-strings.txt"; do
		if grep -F sched_exec_r4l_ "$disabled_file" >> "$arch_out/forbidden-disabled-artifacts.txt"; then
			die "$label disabled object contains an R4 artifact"
		fi
	done
	: > "$arch_out/forbidden-disabled-artifacts.txt"

	key_size=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_key_size)
	bucket_size=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_bucket_size)
	projection_size=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_projection_size)
	rq_state_size=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_rq_state_size)
	b_max=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_b_max_value)
	private_limit=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_private_rq_limit_value)
	key_alignment=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_key_alignment_value)
	bucket_alignment=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_bucket_alignment_value)
	projection_alignment=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_projection_alignment_value)
	rq_state_alignment=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_rq_state_alignment_value)
	worst_private=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_worst_private_bytes_per_rq_value)
	...
	[ "$bucket_size" -le 384 ] || die "$label bucket/notifier exceeds 384 bytes: $bucket_size"
	[ "$projection_size" -le 960 ] || die "$label projection/dirty exceeds 960 bytes: $projection_size"
	[ "$rq_state_size" -le 576 ] || die "$label rq state/bridge exceeds 576 bytes: $rq_state_size"
	[ "$b_max" = 64 ] || die "$label B_max mismatch: $b_max"
	[ "$private_limit" = 65536 ] || die "$label private hard limit mismatch: $private_limit"
	[ "$worst_private" = "$((b_max * projection_size + rq_state_size))" ] || die "$label private-memory arithmetic mismatch"
	[ "$worst_private" -le 62016 ] || die "$label planned R4 envelope exceeded: $worst_private"
	[ "$worst_private" -le "$private_limit" ] || die "$label hard private-memory envelope exceeded: $worst_private"
	...
	for pair in $key_offsets; do
		...
		actual=$(symbol_value "$arch_out/private-on-private.tsv" "sched_exec_r4l_key_${field}_offset_plus_one")
		actual=$((actual - 1))
		[ "$actual" = "$expected" ] || die "$label key.$field offset: $actual"
	done
	inner_offset=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_projection_inner_cfs_rq_offset_plus_one); inner_offset=$((inner_offset - 1))
	outer_offset=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_projection_outer_entity_offset_plus_one); outer_offset=$((outer_offset - 1))
	dirty_node_offset=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_projection_dirty_node_offset_plus_one); dirty_node_offset=$((dirty_node_offset - 1))
	rq_outer_offset=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_rq_state_outer_cfs_rq_offset_plus_one); rq_outer_offset=$((rq_outer_offset - 1))
	dispatch_offset=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_rq_state_dispatch_irq_work_offset_plus_one); dispatch_offset=$((dispatch_offset - 1))
	recovery_offset=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_rq_state_recovery_work_offset_plus_one); recovery_offset=$((recovery_offset - 1))
	dirty_head_offset=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_rq_state_dirty_head_offset_plus_one); dirty_head_offset=$((dirty_head_offset - 1))
	notifier_offset=$(symbol_value "$arch_out/private-on-private.tsv" sched_exec_r4l_bucket_notifier_work_offset_plus_one); notifier_offset=$((notifier_offset - 1))
	...
	[ "$dirty_node_offset" -ge "$((outer_offset + sched_entity_size))" ] || die "$label dirty node overlaps outer entity"
	[ "$dirty_node_offset" -lt "$projection_size" ] || die "$label dirty node exceeds projection"
	[ "$rq_outer_offset" = 0 ] && [ "$cfs_rq_size" -le "$rq_state_size" ] || die "$label private outer cfs_rq does not fit"
	[ "$dispatch_offset" -ge "$cfs_rq_size" ] || die "$label irq work overlaps outer cfs_rq"
	[ "$recovery_offset" -gt "$dispatch_offset" ] || die "$label recovery work order mismatch"
	[ "$dirty_head_offset" -gt "$recovery_offset" ] || die "$label dirty head order mismatch"
	[ "$notifier_offset" -lt "$bucket_size" ] || die "$label notifier exceeds bucket"
	...
	jq -n \
		...
		--argjson private_limit "$private_limit" --argjson key_alignment "$key_alignment" \
		--argjson bucket_alignment "$bucket_alignment" --argjson projection_alignment "$projection_alignment" \
		--argjson rq_state_alignment "$rq_state_alignment" --argjson notifier_offset "$notifier_offset" \
		--argjson dirty_node_offset "$dirty_node_offset" --argjson dispatch_offset "$dispatch_offset" \
		--argjson recovery_offset "$recovery_offset" --argjson dirty_head_offset "$dirty_head_offset" \
...
progress '25% preparing and building arm64 candidate R4-off'
...
progress '33% preparing and building arm64 candidate R4-on'
...
progress '47% validating arm64 51-value invariance and R4 private envelope'
...
progress '67% preparing and building x86_64 candidate R4-off'
...
progress '75% preparing and building x86_64 candidate R4-on'
...
progress '90% validating x86_64 51-value invariance and R4 private envelope'
...
jq -e '.status == "passed" and .architecture == "arm64" and .existing_probe_symbol_count == 51 and .existing_probe_values_changed == 0 and .private_probe_symbol_count == 58 and .ordinary_layout_delta == {sched_entity:0,cfs_rq:0,rq:0,task_struct:0} and .private_layout_envelope_passed == true' "$ARM_OUT/result.json" >/dev/null
jq -e '.status == "passed" and .architecture == "x86_64" and .existing_probe_symbol_count == 51 and .existing_probe_values_changed == 0 and .private_probe_symbol_count == 58 and .ordinary_layout_delta == {sched_entity:0,cfs_rq:0,rq:0,task_struct:0} and .private_layout_envelope_passed == true' "$X86_OUT/result.json" >/dev/null
