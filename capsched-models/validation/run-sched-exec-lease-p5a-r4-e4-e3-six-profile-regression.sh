...
export KBUILD_BUILD_HOST=r4-e4-e3-regression
...
SOURCE_GATE_SOURCE=${E4_SOURCE_GATE_RESULT:-}
...
OUT_ROOT="$WORKSPACE_DIR/build/source-check/sched-exec-lease-p5a-r4-e4-e3-six-profile-regression"
...
cleanup()
{
	...
	if git -C "$LINUX_DIR" worktree list --porcelain 2>/dev/null |
		grep -Fxq "worktree $CANDIDATE_DIR"; then
		git -C "$LINUX_DIR" worktree remove --force "$CANDIDATE_DIR" >/dev/null 2>&1 || true
	fi
	# Apple Container may unregister a shared-directory worktree before an
	# ENOTEMPTY return. Remove only this run-owned path, prune, and verify.
	if [ -e "$CANDIDATE_DIR" ] || [ -L "$CANDIDATE_DIR" ]; then
		rm -rf -- "$CANDIDATE_DIR"
	fi
	git -C "$LINUX_DIR" worktree prune >/dev/null 2>&1 || true
	if [ -e "$CANDIDATE_DIR" ] || [ -L "$CANDIDATE_DIR" ] ||
		git -C "$LINUX_DIR" worktree list --porcelain 2>/dev/null |
		grep -Fxq "worktree $CANDIDATE_DIR"; then
		printf 'error: could not retire diagnostic worktree: %s\n' "$CANDIDATE_DIR" >&2
		cleanup_failed=1
	fi
...
case "$SOURCE_GATE_SOURCE" in
	"$WORKSPACE_DIR"/build/source-check/sched-exec-lease-p5a-r4-e4-local-quantum-source-gate/*/result.json) ;;
	*) die 'E4_SOURCE_GATE_RESULT is outside the exact source-gate result boundary' ;;
esac
if [ ! -f "$SOURCE_GATE_SOURCE" ] || [ -L "$SOURCE_GATE_SOURCE" ]; then
	die 'E4 source-gate result is not a regular non-symlink file'
fi
SOURCE_GATE_SHA=$(sha256sum "$SOURCE_GATE_SOURCE" | awk '{print $1}')
...
if [ -e "$CANDIDATE_DIR" ] || [ -L "$CANDIDATE_DIR" ]; then
	die "diagnostic worktree already exists: $CANDIDATE_DIR"
fi

mkdir -p "$OUT_ROOT" "$(dirname "$BUILD_ROOT")" "$(dirname "$CANDIDATE_DIR")"
...
git -C "$LINUX_DIR" worktree add --detach "$CANDIDATE_DIR" "$CANDIDATE_COMMIT" > "$OUT_DIR/e3-worktree-add.log" 2>&1
[ "$(git -C "$CANDIDATE_DIR" rev-parse HEAD)" = "$CANDIDATE_COMMIT" ] || die 'diagnostic worktree commit mismatch'
[ "$(git -C "$CANDIDATE_DIR" rev-parse 'HEAD^{tree}')" = "$CANDIDATE_TREE" ] || die 'diagnostic worktree tree mismatch'
git -C "$CANDIDATE_DIR" diff --binary "$CANDIDATE_PARENT..$CANDIDATE_COMMIT" > "$OUT_DIR/e3-source.diff"
[ "$(sha256sum "$OUT_DIR/e3-source.diff" | awk '{print $1}')" = "$CANDIDATE_DIFF_SHA" ] || die 'diagnostic source diff changed'
...
configure_boot()
{
	...
	run_make_step "$label defconfig" "$OUT_DIR/$label-defconfig.log" "$OUT_DIR/$label-defconfig-verification.log" \
		make -C "$CANDIDATE_DIR" O="$out" ARCH="$arch" CROSS_COMPILE="$cross" defconfig
	...
	run_make_step "$label olddefconfig" "$OUT_DIR/$label-olddefconfig.log" "$OUT_DIR/$label-olddefconfig-verification.log" \
		make -C "$CANDIDATE_DIR" O="$out" ARCH="$arch" CROSS_COMPILE="$cross" olddefconfig
	for required in \
	...
	grep -Fxq '# CONFIG_SCHED_EXEC_LEASE_R4_MEASURE_KUNIT_TEST is not set' \
		"$out/.config" || die "$label enabled the E4 measurement suite"
...
build_image()
{
	...
	mkfifo "$fifo"
	set +e
	make -C "$CANDIDATE_DIR" O="$out" ARCH="$arch" CROSS_COMPILE="$cross" -j"$JOBS" "$target" > "$fifo" 2>&1 &
	make_pid=$!
	...
	if has_clock_skew "$log"; then
		...
		make -C "$CANDIDATE_DIR" O="$out" ARCH="$arch" CROSS_COMPILE="$cross" -j"$JOBS" "$target" > "$verification_log" 2>&1
		! has_compiler_diagnostic "$verification_log" || die "$label verification compiler diagnostic"
...
if [ "${CONFIG_SMOKE_ONLY:-0}" = 1 ]; then
	...
	jq -n --arg run_id "$RUN_ID" --arg runner_sha "$runner_initial_sha" --arg classifier_sha "$WARNING_CLASSIFIER_SHA" --arg candidate "$CANDIDATE_COMMIT" --arg source_gate_sha "$SOURCE_GATE_SHA" --arg closure_r3_sha "$CLOSURE_R3_SHA" --arg closure_r4_sha "$CLOSURE_R4_SHA" --arg rejection_sha "$SIX_BOOT_ATTEMPT_1_REJECTION_SHA" --arg rejection_2_sha "$SIX_BOOT_ATTEMPT_2_REJECTION_SHA" --arg rejection_3_sha "$SIX_BOOT_ATTEMPT_3_REJECTION_SHA" --argjson retries "$clock_skew_retries" --argjson ledger_selftest "$receipt_ledger_selftest_passed" --argjson classifier_selftest "$warning_classifier_selftest_passed" \
		'{schema_version:1,status:"passed_e4_candidate_six_profile_e3_config_smoke_without_build_or_boot",run_id:$run_id,runner_sha256:$runner_sha,kernel_warning_classifier_sha256:$classifier_sha,candidate_commit:$candidate,source_gate_result_sha256:$source_gate_sha,predecessor_source_gate_closure_result_sha256:[$closure_r3_sha,$closure_r4_sha],prior_six_boot_attempt_rejection_sha256:$rejection_sha,prior_six_boot_attempt_2_rejection_sha256:$rejection_2_sha,prior_six_boot_attempt_3_rejection_sha256:$rejection_3_sha,receipt_ledger_jsonl_selftest_passed:($ledger_selftest == 1),kernel_warning_classifier_selftest_passed:($classifier_selftest == 1),unknown_kcsan_messages_fail_closed:true,full_six_profile_e3_regression_required:true,configs:["arm64_standard_debug","x86_64_standard_debug","arm64_hotplug_fault_injection","x86_64_hotplug_fault_injection","arm64_generic_kasan","x86_64_kcsan"],clock_skew_retries:$retries,builds_started:0,boots_started:0,e4_measurement_suite_enabled:false,timing_measurement_may_start:false,r4_e4_source_accepted:false,production_protection:false,datacenter_ready:false}' > "$OUT_DIR/config-smoke-result.json"
...
progress '97% sealing complete six-profile E3 regression and negative claims'
...
[ -z "$(git -C "$CANDIDATE_DIR" status --porcelain --untracked-files=no)" ] || die 'source worktree changed during matrix'
...
jq -n \
	--arg run_id "$RUN_ID" --arg candidate "$CANDIDATE_COMMIT" --arg parent "$CANDIDATE_PARENT" --arg tree "$CANDIDATE_TREE" --arg diff_sha "$CANDIDATE_DIFF_SHA" \
	...
	'{schema_version:1,id:"sched-exec-lease-p5a-r4-e4-e3-six-profile-regression-result-v1",run_id:$run_id,status:"passed_six_profile_e3_regression_awaiting_independent_closure",candidate_commit:$candidate,candidate_parent:$parent,candidate_tree:$tree,candidate_diff_sha256:$diff_sha,primary_commit:$primary,patch_queue_commit:$patch_queue,source_gate_result:$source_gate,source_gate_result_sha256:$source_gate_sha,predecessor_source_gate_closure_result_sha256:[$closure_r3_sha,$closure_r4_sha],prior_six_boot_attempt_rejection_sha256:$rejection_sha,prior_six_boot_attempt_2_rejection_sha256:$rejection_2_sha,prior_six_boot_attempt_3_rejection_sha256:$rejection_3_sha,receipt_ledger_jsonl_selftest_passed:($ledger_selftest == 1),kernel_warning_classifier_sha256:$classifier_sha,kernel_warning_classifier_selftest_passed:($classifier_selftest == 1),unknown_kcsan_messages_fail_closed:true,runner:$runner,runner_sha256:$runner_sha,plan:$plan,plan_sha256:$plan_sha,architectures:["arm64","x86_64"],profiles:["arm64_standard_debug","x86_64_standard_debug","arm64_hotplug_fault_injection","x86_64_hotplug_fault_injection","arm64_generic_kasan","x86_64_kcsan"],suite:"sched_exec_lease_r4_concurrency",required_cases_per_profile:36,passed_cases_per_profile:36,total_passed_cases:216,receipts_per_profile:36,total_receipts:216,stress_families:["bridge","notifier","migration","hotplug","retirement"],stress_iterations_per_family_per_profile:2048,allocation_fault_sites:6,case_failures:0,case_skips:0,case_timeouts:0,warning_reports:0,build_clock_skew_retries:$clock_skew_retries,final_build_clock_skew_warnings:0,matrix_reduction:false,fresh_build_output_per_profile:true,sequential_build_retirement:true,compiler_config_image_object_qemu_ktap_console_seed_fault_receipts_recorded:true,profile_results:$boot_results,profile_results_sha256:$boot_results_sha,results:$results[0],six_profile_e3_regression_passed:true,independent_regression_closure_pending:true,predecessor_r4_e3_accepted:true,e4_measurement_suite_enabled:false,timing_measurement_may_start:false,r4_e4_source_accepted:false,primary_linux_changed:false,patch_queue_changed:false,real_scheduler_attachment:false,runtime_scheduler_hook_approved:false,runtime_behavior_approved:false,runtime_denial_correctness:false,monitor_delivery_or_enforcement:false,cross_class_coverage:false,bounded_wall_clock_latency_claim:false,performance_claim:false,cost_claim:false,production_protection:false,deployment_ready:false,multi_node_ready:false,multi_cluster_ready:false,datacenter_ready:false}' > "$OUT_DIR/result.json.pending"
	jq -e '.status == "passed_six_profile_e3_regression_awaiting_independent_closure" and .receipt_ledger_jsonl_selftest_passed == true and .kernel_warning_classifier_selftest_passed == true and .unknown_kcsan_messages_fail_closed == true and .profiles == ["arm64_standard_debug","x86_64_standard_debug","arm64_hotplug_fault_injection","x86_64_hotplug_fault_injection","arm64_generic_kasan","x86_64_kcsan"] and .total_passed_cases == 216 and .total_receipts == 216 and .case_failures == 0 and .case_skips == 0 and .case_timeouts == 0 and .warning_reports == 0 and .six_profile_e3_regression_passed == true and .independent_regression_closure_pending == true and .e4_measurement_suite_enabled == false and .timing_measurement_may_start == false and .r4_e4_source_accepted == false and .production_protection == false and .datacenter_ready == false' "$OUT_DIR/result.json.pending" >/dev/null
mv "$OUT_DIR/result.json.pending" "$OUT_DIR/result.json"
sha256sum "$OUT_DIR/result.json" > "$OUT_DIR/result.sha256"
progress '100% exact six-profile E3 regression passed; independent closure still required'
printf 'result=%s\n' "$OUT_DIR/result.json"
