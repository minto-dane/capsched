...
| 0320 | Exact G6 run `candidate4-full-20260812T170823Z` durably finalizes `RAW_CAPTURE_INCOMPLETE` after the child reaches its isolated 7.5-GiB limit; the successor uses compact exact state columns and fixed-width frontier/history indices, reduces the measured child prefix RSS by 34.5123%, crosses the prior parent failure boundary, and repairs controller-fence publication plus authenticated owner-failure snapshots while G6/G7 and all external claims remain open | Dynamic Residency F0 Candidate-4 G6 Compact State Store and Owner-Failure Snapshot |
