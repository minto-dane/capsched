...
| 0069 | Safe model passed; unsafe models produced expected counterexamples | Monitor Admission Carrier Sketch TLC |
...
| 0141 | Passed for P4 pre-entry replay, drift, diff/security review, generated-code review, and broader QEMU matrix; P4 implementation not applied | SchedExecLease P4 Pre-Entry Validation |
| 0142 | Executed; hardened drift gate found global D4 stale device/QueueLease drift, so P4 remains paused pending scoped drift closure and anchor evidence | SchedExecLease P4 Pre-Implementation Critical Audit Validation |
...
| 0157 | Passed for full `vmlinux` CONFIG_SCHED_EXEC_LEASE=off/on build; later object/upstream/QEMU/final evidence is recorded in 0158-0161 | SchedExecLease P5A0.P1 Full vmlinux Build |
| 0158 | Passed for object/symbol/section-size review, hot scheduler function-size review, and build-only task layout probe; later upstream/QEMU/final evidence is recorded in 0159-0161 | SchedExecLease P5A0.P1 Object and Layout Evidence |
...
| 0291 | Exact 20-scenario pre-formal candidate `24bb1320...174f` passes strict contract/object-set validation, 8,119 malformed witness type/container rejections without executor crashes, 322 intended-gate hostile mutations, and 67 assurance-fixture mutations; real assurance exits 78 unimplemented, so freeze, TLA+, model support, and protection remain false | Dynamic Residency Pre-Formal Internal Consistency |
| 0292 | Four fresh independent internal hostile reviews return `FREEZE_NO` for exact candidate `24bb1320...174f`; 21 canonical blockers reject semantic freeze and reopen architecture-first redesign while preserving 0291 only as scoped internal regression evidence | Dynamic Residency Fresh Hostile Review Rejection |
| 0293 | Exact materialized R6 v2 `24887ac2...a62d` is reproducible but four independent reviews return `FREEZE_NO`; strict validation and mutation baseline both fail on a retained stale proof-ledger node, and 40 normalized findings require an executable R7 semantic IR before another review or TLA+ | Dynamic Residency R6 Successor Hostile Rejection |
| 0294 | Four independent reviews reject exact R7 pre-IR snapshot `6d08ec43...e8ec` with 41 activation, failure, transfer, progress, scale, and formal-closure blockers; R8 architecture-first redesign is required before semantic IR or TLA+ | Dynamic Residency R7 Pre-IR Hostile Rejection |
| 0295 | Four independent reviews return `IR_ENCODING_READY=NO` and `FREEZE_NO` for exact R8 snapshot `4f30f2b8...7b5b`; 54 raw findings normalize to 40 local blockers, rejecting IR/TLA+ and requiring a clean R9 architecture | Dynamic Residency R8 Hostile Review Rejection |
| 0296 | Four exact reviews reject R9 snapshot `f7e9f3df...11362`; 78 raw findings normalize losslessly to 42 blockers because the apparent registries omit exact state, formulas, actions, ownership, recovery, causality, and distributed lifecycle semantics; R10 typed machine architecture is required before freeze or TLA+ | Dynamic Residency R9 Hostile Review Rejection |
| 0297 | Concurrent hostile tooling audit rejects R10 closure/readiness/freeze after 18 accepted adversarial variants expose 14 status, transitive-read, recovery, AST, write, proof, import, metadata, object, instance, provider/rank, bundle, and standalone-IR failures; this is negative-only evidence pending executable mutations and a fresh exact review | Dynamic Residency R10 Machine-IR Tooling Hostile Rejection |
| 0298 | All 29 local structural hostile mutations reject at their expected IDs, but independent assurance remains rejected because the harness shares validator code and candidate expectations, has a file-read TOCTOU, lacks semantic-vacuity and harness meta-mutants, and has no external promotion manifest | Dynamic Residency R10 Assurance Trust-Boundary Rejection |
| 0299 | Exact snapshot `ad463ae...6f97` passes an external-catalog, snapshot-once campaign: two accepted baselines, all 58 executions of 29 structural mutants rejected at exact IDs, and six aggregation/hash meta-mutants killed; authority remains development-regression-only pending semantic-vacuity, import, runtime, review, and promotion closure | Dynamic Residency R10 Independent Structural Hostile Campaign |
| 0300 | Exact R10 baseline accepts all 13 semantic-vacuity variants: literal-true conservation, empty proof support, fabricated claims/blockers, prepublished provider facts, vacuous provider/progress contracts, weakened finite profiles and CD metadata, inflated atomicity, and authority demotion; semantic closure and TLA+ remain rejected | Dynamic Residency R10 Semantic-Vacuity Rejection |
| 0301 | Exact R10 structural regressions pass but 26/42 blockers remain pending and 13/13 semantic-vacuity variants are accepted; R10 is rejected and retained, while ADR-0016 authorizes only policy-derived modular R11 architecture construction through D0-D17 and W0-W12 before any freeze or TLA+ | Dynamic Residency R10 Exact Rejection and R11 Construction Gate |
| 0302 | Exact 17-artifact R11 G0 local bundle passes snapshot verification; all 12 policy/profile/protocol mutations reject and two repeated runs are byte-identical, but external reviews and gate decision are absent so G0 and IR/TLA+ authorization remain false | Dynamic Residency R11 G0 Local Candidate Integrity |
| 0303 | Exact R11 G0 v1 retains local integrity but is rejected before external review: all 12 semantic-vacuity variants pass the checker, all five forged-positive review/gate conditions pass their schemas, repeated outputs are byte-identical, and three local hostile reviews reject security, formal, and composition readiness | Dynamic Residency R11 G0 v1 Hostile Rejection |
| 0304 | The first epoch-2 kernel v2 passes only strict schema/inventory checks and is rejected before interpreter work: three local reviews find absent denotation, circular gates, incomplete phase/totality/fairness/noninterference, circular assurance roots, and zero executable mandatory interaction families | Dynamic Residency Epoch-2 Kernel v2 Hostile Rejection |
| 0305 | Exact v3 structural inventories reproduce with one deterministic local output, but four exact-hash local reviews reject model/type/action/frame closure, liveness/time/durability/distribution/composition semantics, physical threat refinement, and K0 assurance closure; only F0 v4 design is authorized | Dynamic Residency Semantic Foundation v3 Hostile Rejection |
| 0306 | Exact F0 v4 identity and 21/21 structural/authority mutations pass, but four exact-hash local reviews reject term/event construction, invariant/action parameter typing, action closure, footprint and state granularity, inhabited claim scopes, extension/refinement morphisms, and the proof boundary; only F0 v5 design is authorized | Dynamic Residency F0 v4 Hostile Rejection |
| 0307 | F0 v5 machine draft-1 passes only local structural checks and 38/38 structural mutations; Markdown/AST parity is rejected and no semantic promotion is authorized | Dynamic Residency F0 v5 Machine Draft-1 Structural and Parity Disposition |
| 0308 | Immutable source-static and link-construction regression passes grammar 61/61, static rules 83/83, wire 23/23, Core 51/51, link-rule fields 56/56, and local cross-verifier 21/21; Eval, Reads, transition, WF, external review, F0, and G0 remain false | Dynamic Residency F0 v5 Static and Link Construction Regression |
| 0309 | Source-derived occurrence and immutable request/context regressions cover EvalF construction and hostile metadata/input cases, but captured-code execution, whole-pipeline metering, external supervision, genuine Unsupported coverage, metatheory, F0, and G0 remain false | Dynamic Residency F0 v5 Checked Evaluation Regression |
| 0310 | Supervisor v2 local checker explores 82,944 final evidence products with no positive-guard overlap, 16 required outcomes, three frame/exit/quota order pairs, and 97 hostile mutations; implementation, Linux refinement, checker soundness, fresh review, taxonomy closure, F0, and G0 remain false | Dynamic Residency F0 v5 Supervisor v2 Local Machine Regression |
| 0311 | Three fresh local reviews reproduce but reject v2's local pass as operational-LTS, semantic-verdict, Linux-refinement, or assurance evidence; only normalized truth-table and snapshot-drift credit remains, and v3 requires effectful reachability, typed receipts, role separation, linearization, and three-chain publication | Dynamic Residency F0 v5 Supervisor v2 Hostile Rejection |
| 0312 | Exact executable v3 draft-0 reaches 10,648 states per role but has 273 nonterminal deadlocks, 686 prefixes without a terminal path, zero exact registry/action matches, and three independent hostile-review rejections; only counterexample-producing skeleton credit remains | Dynamic Residency F0 v5 Supervisor v3 Draft-0 Hostile Rejection |
| 0313 | Candidate-4 pre-full local regressions pass child 274, parent 715, and runner 44 hostile cases plus the fast validator; role agreement, reachable commutation membership, realizable cardinality and edge bounds, and invalid-launch regressions are bound, while full child/parent reachability and declared commutation remain not run, all external claims remain open, and F0/R11/G0/protection authority remains false | Dynamic Residency F0 v5 Supervisor v3 Candidate-4 Pre-Full Local Closure |
| 0314 | Exact authority-disjoint capture contract `14ca4b...fd075` passes strict validation, rejects 153 hostile semantic mutations, and passes 13 independently derived consistency checks across authority, typed objects, Linux primitives, split lifecycles, receipts, resources, invariants, gates, and authorization; G1/G2 close locally | Dynamic Residency F0 Candidate-4 Authority-Disjoint Capture Contract |
| 0315 | Dedicated arm64 VM mechanism regressions pass strict C build, launcher basic, 10 launcher-hostile, five snapshot-hostile, read-only EROFS reuse, five-component supervisor, three guardian, and three reducer cases; G3-G5 close locally while exact long capture G6 and real-byte reduction G7 remain open | Dynamic Residency F0 Candidate-4 Authority-Disjoint Capture Mechanism |
| 0316 | Exact G6 run `candidate4-full-20260811T210659Z` finalized `RAW_CAPTURE_INCOMPLETE` after child action-effect rejection; root capture, kill-and-drain, immutable commit, and fail-closed positive-ineligibility worked, while G6 stays open and G7 blocked | Dynamic Residency F0 Candidate-4 G6 Incomplete Disposition |
| 0317 | The OBS-032 hidden-work effect declaration and targeted regression are repaired; 275/715/44 fast cases, clean install, EROFS reuse, three reducer boundaries, nine state-drift mutations, and home-unshared VM-native exact-input transfer pass, making a fresh G6 retry eligible while G6/G7 and all external claims remain open | Dynamic Residency F0 Candidate-4 G6 Counterexample Repair |
| 0318 | Exact G6 run `candidate4-full-20260811T223707Z` durably finalized guardian-incomplete after unit-wide OOM propagation; the resource-repaired exact inputs preserve semantics, pass 275/715/44 fast, 12 model-memory, seven capture-resource, and three reducer-binding cases, and contract `0a6954...c6d` with 155 hostile plus 14 derived checks; clean commit `a956de2...` and all short mechanisms pass, making a fresh G6 retry eligible while G6/G7 and all external claims remain open | Dynamic Residency F0 Candidate-4 G6 Resource Repair |
| 0319 | Exact G6 run `candidate4-full-20260812T134610Z` proves component-OOM isolation and durable fail-closed finalization but again reaches 7.5 GiB; the successor preserves semantics with persistent exact child/parent histories and a collision-safe fixed-width state index, passes 275/715/44 fast plus 19 memory-policy cases, and is clean-installed from `1fccaca...` under manifest `bd86819a...` with the full short suite passing; a fresh G6 retry is eligible while G7 remains blocked | Dynamic Residency F0 Candidate-4 G6 Persistent Exact Frontier |
| 0320 | Exact G6 run `candidate4-full-20260812T170823Z` durably finalizes `RAW_CAPTURE_INCOMPLETE` after the child reaches its isolated 7.5-GiB limit; the successor uses compact exact state columns and fixed-width frontier/history indices, reduces the measured child prefix RSS by 34.5123%, repairs controller-fence publication plus authenticated owner-failure snapshots, and is clean-installed from `1c076a9...` under manifest `75d17ee...` with the complete short suite passing; a fresh G6 retry is eligible while G7 and all external claims remain open | Dynamic Residency F0 Candidate-4 G6 Compact State Store and Owner-Failure Snapshot |
| 0321 | Exact G6 run `candidate4-full-20260812T184826Z` durably finalizes `RAW_CAPTURE_INCOMPLETE` after the child again reaches its isolated 7.5-GiB limit; the successor preserves Python transition semantics while packing exact receipt/history values, matches old child/parent BFS prefixes field-for-field, reduces their RSS by 53.0919%/56.7672%, and passes 284/739/44 fast plus 30 memory-policy cases; clean install remains required before a fresh G6 retry and G7 stays blocked | Dynamic Residency F0 Candidate-4 G6 Packed Exact History Arena |
| 0322 | Exact G6 run `candidate4-full-20260812T231441Z` durably finalizes `RAW_CAPTURE_INCOMPLETE` after 4.92 hours at the isolated 7.5-GiB limit; the successor preserves exact semantics while moving fixed-width state/index/graph storage to unlinked mmap files on a capped direct-I/O loop ext4 filesystem, matches a 20,510-state RAM prefix exactly, passes the integrated mechanism/recovery suite, and requires clean install before retry | Dynamic Residency F0 Candidate-4 G6 Disk-Backed Exact Enumerator |
| 0323 | Exact disk-backed G6 run `candidate4-full-20260813T071653Z` allocates 11.38 GiB spill with zero OOM kills, then durably rejects a 17-action pending-ATTACH/protection-closure contradiction; the repaired child retains explicit reject/breach outcomes and passes 285 hostile cases, but requires clean install and the complete short suite before retry | Dynamic Residency F0 Candidate-4 G6 Pending-Hostile Closure Repair |
| 0324 | Exact disk-backed G6 run `candidate4-full-20260813T223812Z` reaches its sealed 43,200-second child deadline with zero OOM kills and complete cleanup; the successor's nine-case behavioral-quotient boundary, 295/739/48 hostile suites, 33 memory-policy cases, and exact/quotient storage equivalence pass, while weaker authority erasure is rejected and implementation refinement remains open | Dynamic Residency F0 Candidate-4 Behavioral Audit-Representation Quotient |
| 0325 | Two root-owned reconstructions of one reviewed commit exposed checkout-path-dependent launcher debug bytes; canonical file/debug/macro prefix maps plus date-time rejection now produce byte-identical launchers from two distinct source and output roots, with clean reinstall still required | Dynamic Residency F0 Candidate-4 Reproducible TCB Build |
| 0326 | Exact behavioral-quotient G6 run `candidate4-full-20260814T130141Z` durably finalizes `RAW_CAPTURE_INCOMPLETE` after a one-PID CPython child uses essentially one CPU for the sealed 43,200-second deadline; peak memory is 4.66 GB with zero limit/OOM events, private ext4 cleanup and immutable commit pass, and G6 replay is blocked pending deterministic Python/Rust differential refinement | Dynamic Residency F0 Candidate-4 Rust Execution-Refinement Boundary |
| 0327 | Zero-dependency Rust matches both Python child roles byte-for-byte for setup, 1,000-source exact prefixes, and 17 traces covering all 56 actions; 10,000-source counts/multiplicities, forced collision resolution, 14 checkpoint mutations, 107-MiB bounded RSS, and a two-root reproducible binary pass, while parent/exhaustive/hostile/multiworker/integration/install gates remain open and G6 retry stays disabled | Dynamic Residency F0 Candidate-4 Rust Child-Transition Refinement Checkpoint |
| 0328 | Independent Rust grant/evidence/recovery/decision/InstanceWF checks match Python across 12,213 initial/successor evaluations per role and validate every intermediate/outgoing all-action trace state; eight unit tests, 16 checkpoint mutations, and a byte-identical two-root binary pass, while six hostile/exhaustive/multiworker/parent/integration/install gates remain open | Dynamic Residency F0 Candidate-4 Rust Independent Well-Formedness Refinement |
| 0329 | Exact grant/full-state fixtures map 59/295 normative child hostile credits to byte-identical Python/Rust WF results; 11 malformed transports, 10 Rust unit tests, the unchanged 295-case Python suite, 19 checkpoint mutations, and a two-root reproducible binary pass while full hostile parity remains open at 236 credits | Dynamic Residency F0 Candidate-4 Rust Hostile-WF Fixture Slice |
| 0330 | Exact before/action/actor/after fixtures extend the Python/Rust differential to 81 queries and conservatively map 71/295 original credits; full-state round trips, 12 hostile plus 10 supplemental positive edges, 15 malformed rejections, 10 Rust tests, 23 checkpoint mutations, all bounded regressions, and a two-root reproducible binary pass while 224 credits and six gates remain open | Dynamic Residency F0 Candidate-4 Rust Exact-Edge Fixture Slice |
| 0331 | Guarded successor fixtures compare exact action/actor multisets and distinguish invalid-state rejection from terminal emptiness across 27 additional states; the 108-query corpus maps 98/295 credits, rejects 18 malformed transports, preserves the 295-case Python suite, and reproduces one zero-dependency binary while 197 credits and six gates remain open | Dynamic Residency F0 Candidate-4 Rust Guarded-Successor Fixture Slice |
