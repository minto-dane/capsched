...
CONFIG="$CAPSCHED_DIR/capsched-models/analysis/sched-exec-lease-p5a-r-picker-ineligibility-gate-v1.json"
MODEL_DIR="$CAPSCHED_DIR/capsched-models/formal/0104-p5a-r-picker-ineligibility-gate-model"
MODEL="P5ARPickerIneligibilityGate.tla"
TLA_JAR=${TLA_JAR:-/home/nia/tools/tla/tla2tools.jar}
RUN_ID=${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}
OUT_DIR="$WORKSPACE_DIR/build/source-check/sched-exec-lease-p5a-r-picker-ineligibility-gate/$RUN_ID"
...
jq -e '
	...
	(.formal.unsafe_cfg_count == 28) and
	(.formal.unsafe_expected_counterexamples == 28) and
	(.rejected_design_families | length >= 20)
' "$CONFIG" >/dev/null
...
(
	cd "$MODEL_DIR"
	java -cp "$TLA_JAR" tlc2.TLC -deadlock -config P5ARPickerIneligibilityGateSafe.cfg "$MODEL"
...
unsafe_expected=0
unsafe_fail=0
for cfg in "$MODEL_DIR"/P5ARPickerIneligibilityGateUnsafe*.cfg; do
	name=$(basename "$cfg" .cfg)
	log="$OUT_DIR/tlc-$name.log"
	if (
		cd "$MODEL_DIR"
		java -cp "$TLA_JAR" tlc2.TLC -deadlock -config "$(basename "$cfg")" "$MODEL"
	) > "$log" 2>&1; then
...
cfg_count=$(find "$MODEL_DIR" -maxdepth 1 -name 'P5ARPickerIneligibilityGateUnsafe*.cfg' | wc -l)
if [ "$unsafe_expected" -ne 28 ] || [ "$cfg_count" -ne 28 ]; then
	echo "unsafe counterexample count mismatch: expected=28 actual=$unsafe_expected cfg_count=$cfg_count" >&2
	exit 1
fi
