...
| 0230 | The OOM-isolated retry proves fail-closed supervisor survival but again reaches the 7.5-GiB child bound; immutable persistent child/parent receipt chains and a fixed-width open-addressed state index remove repeated prefix arrays and Python dict entries while full collision equality, exact state/history identity, CSR edges, zero swap, transitions, and production hot paths remain unchanged | Dynamic Residency F0 Candidate-4 G6 Persistent Exact Frontier |
| 0231 | The third OOM-isolated retry again reaches the sealed 7.5-GiB child bound; a columnar compact exact state store, fixed-width BFS frontier, and exact indexed history counting reduce the measured child prefix RSS by 34.5123% without quotienting identity, while a controller-fence guard and signed owner-failure snapshot close two independent parent semantic gaps | Dynamic Residency F0 Candidate-4 G6 Compact Exact State Store and Owner-Failure Snapshot |
...
| 0236 | The tenth G6 attempt proves the quotient reduced peak memory to 4.66 GB and spill allocation to 3.96 GB without OOM, but a one-PID CPython child still consumes essentially one CPU through the 43,200-second deadline; byte-identical replay is blocked while Python remains the normative oracle and a deterministic, pinned, reproducible Rust accelerator must pass state/transition/projection/hostile differential refinement before capture eligibility | Dynamic Residency F0 Candidate-4 Rust Execution-Refinement Boundary |
| 0237 | The zero-dependency Rust child transition implementation matches Python byte-for-byte through 1,000 BFS sources and 17 all-action traces, matches 10,000-source cardinalities and action multiplicities, resolves forced digest collisions by full canonical equality, reduces rejected naive RSS by about 92%, and builds reproducibly; parent, exhaustive, hostile, multiworker/external-memory, integration, and clean-install gates remain open so G6 retry stays disabled | Dynamic Residency F0 Candidate-4 Rust Child-Transition Refinement Checkpoint |
| 0238 | Handwritten Rust independently checks grants, ordered authenticated evidence, recovery, decisions, and complete child InstanceWF across 12,213 initial/successor evaluations per role plus every state/edge in 17 all-action traces; this closes only the independent-WF subgate while six hostile/exhaustive/multiworker/parent/integration/install gates remain open | Dynamic Residency F0 Candidate-4 Rust Independent Well-Formedness Refinement |
| 0239 | A bounded exact-state transport maps 59/295 named hostile credits to independent Python/Rust grant, evidence, and InstanceWF evaluation; all result bytes match, 11 malformed transports fail closed, and 236 edge/effect/commutation/representation credits remain explicitly open | Dynamic Residency F0 Candidate-4 Rust Hostile-WF Fixture Slice |
| 0240 | Exact 54-field state round trips and before/action/actor/after queries extend the differential to 81 grant/state/edge cases; 12 hostile edges conservatively advance mapped coverage to 71/295 while 10 positive witnesses remain supplemental, 15 malformed transports fail closed, and six refinement gates stay open | Dynamic Residency F0 Candidate-4 Rust Exact-Edge Fixture Slice |
| 0241 | A guarded successor query compares the canonical action/actor multiset, distinguishes invalid-state rejection from valid terminal emptiness, and covers 27 reacquisition/protection/quota/failover/hostile/seal states; the 108-query corpus maps 98/295 credits with 10 supplemental witnesses and 197 credits open | Dynamic Residency F0 Candidate-4 Rust Guarded-Successor Fixture Slice |
...
| `dynamic-residency-f0-v5-supervisor-v3-candidate4-frontier-repair-v1.json` | Candidate-4 persistent exact frontier repair | Second OOM trigger, exact successor input hashes, persistent ordered histories, fixed-width exact state index with full collision equality, 19-case memory policy, unchanged semantic registry, and all-false external authorization |
| `dynamic-residency-f0-v5-supervisor-v3-candidate4-state-store-repair-v1.json` | Candidate-4 compact exact state-store repair | Third OOM trigger, compact columnar exact states, fixed-width frontier and history index, parent controller-fence and authenticated failure-snapshot repairs, bounded profiles, 24-case memory policy, and all-false external authorization |
| `dynamic-residency-f0-v5-supervisor-v3-candidate4-packed-history-repair-v1.json` | Candidate-4 packed exact history repair | Fourth OOM trigger, reversible packed receipt/history arenas, adaptive exact references, bounded exact caches, dense collision-safe state index, matched child/parent profiles, 30-case memory policy, and all-false external authorization |
| `dynamic-residency-f0-v5-supervisor-v3-candidate4-external-memory-repair-v1.json` | Candidate-4 disk-backed exact enumerator | Fifth OOM trigger, exact unlinked mmap state/index/graph storage, per-component direct-I/O loop ext4 capacity isolation, RAM/spill sequence equivalence, guardian cleanup, and all-false external authorization |
| `dynamic-residency-f0-v5-supervisor-v3-candidate4-pending-attack-closure-repair-v1.json` | Candidate-4 pending-hostile closure repair | No-OOM full-capture semantic trigger, exact 17-action trace, balanced hostile-attempt closure guard, retained rejection/breach alternatives, inherited external-memory boundary, 285/739/44 regressions, and all-false external authorization |
| `f0-c4-rust-child-transition-refinement-checkpoint-v1.json` | Candidate-4 Rust child-transition checkpoint | Exact Python/Rust input hashes, bounded byte-exact graph, independent WF, 98/295 hostile grant/state/edge/successor mapping plus 10 supplemental positive edges and all-action trace results, wide multiplicity diagnostic, collision/full-equality and shared-history policy, reproducible tool/binary identities, eight closed subgates, six explicitly open subgates, and all-false claim authority |
