...
| `f0-c4-rust-child-transition-refinement-checkpoint-v1.json` | Candidate-4 Rust child-transition checkpoint | Exact Python/Rust input hashes, bounded byte-exact graph, independent WF and all-action trace results, wide multiplicity diagnostic, collision/full-equality and shared-history policy, reproducible tool/binary identities, eight closed subgates, six explicitly open subgates, and all-false claim authority |
